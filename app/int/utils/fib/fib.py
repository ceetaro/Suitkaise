...
class FunctionInstanceRegistry:
    ...
    """
    _function_instance_registry_instances = []
    _function_instance_registry_lock = threading.RLock()

    def __new__(cls):
        """
        Create a new instance of FunctionInstanceRegistry or return
        the existing one.
        
        Returns:
            the FunctionInstanceRegistry instance
            
        """
        with cls._function_instance_registry_lock:
            # get domain of calling module
            domain = skdomain.get_domain()
            if domain == skdomain.SKDomain.INTERNAL:
                # check global internal for an existing instance
                if 'internal' not in globals():
                    raise ValueError("Global variable 'internal' not found. "
                                    "Please ensure that the internal execution space is initialized.\n")
                global internal
                if 'function_instance_registry' in internal \
                and internal['function_instance_registry'] is not None:
                    return internal['function_instance_registry']
                
                # create a new instance if it doesn't exist
                instance = super(FunctionInstanceRegistry, cls).__new__(cls)
                cls._init_function_instance_registry()
                cls._function_instance_registry_instances.append(instance)
                return instance

            elif domain == skdomain.SKDomain.EXTERNAL:
                # check global external for an existing instance
                if 'external' not in globals():
                    raise ValueError("Global variable 'external' not found. "
                                    "Please ensure that the external execution space is initialized.\n")
                global external
                if 'function_instance_registry' in external \
                and external['function_instance_registry'] is not None:
                    return external['function_instance_registry']
                ...
                raise ValueError("Unknown domain. Cannot create FunctionInstanceRegistry.\n")
            
    def _init_function_instance_registry(self):
        """
        Initialize a FunctionInstanceRegistry instance.

        This method is called only once per domain when the singleton
        instance for that domain is created.
        
        """
        # registry for function instances
        self.function_instances = {}
        
        print("FunctionInstanceRegistry initialized.")
