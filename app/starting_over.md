...
## SKCereal, for internal and external serialization
...
## SKRoot, IntRoot and ExtRoot

we should create actual pointer objects we can use to signify the directory that holds internal code and the one that holds external code instead of hardcoding them. We should also initialize them with relative paths up to suitkaise root, and let os.abspath handle the absolute path for whatever device we are installed on. additionally, we can create a generalized SKRoot that can be a part of the library.

IntRoot and ExtRoot are application specific objects that signify what directories are internal domain or external domain. We will use an updated SKDomain utility to do this. These will not be part of the suitkaise library.

These objects will be stored in the top level global storage with the Bridge object.
...
It is possible that SKFunctions would need to travel across processes. We need to make sure that they can be correctly serialized.
