...
why the cucumber module was created
...
But if you want to serialize everything else, then you should be using `cucumber`.
...
(start of dropdown 1)
### Things that `cucumber` handles that `pickle`, `cloudpickle`, and `dill` cannot
...
(start of dropdown 2)
### Things that `cucumber` handles the fastest
...
(start of dropdown 3)
### Things that `cucumber` handles faster than all others besides `pickle`
...
Cucumber deconstructs objects down to a level where `pickle` can handle them, using nested dicts and base `pickle` types within.
...
Cucumber then serializes the nested dicts to bytes using `pickle`.
...
With this, `cucumber` can serialize almost anything, even your most complex classes and objects that others cannot.
...
The WorstPossibleObject is a comprehensive test of `cucumber`'s ability to serialize and deserialize complex objects.
...
class WorstPossibleObject:
    ...
    def init_all_complex_types_in_random_order(self):
        """Initialize all complex, unpickleable objects that cucumber handlers must handle."""
...
`cucumber` can serialize and deserialize these WorstPossibleObjects hundreds of times in a row. 
