...
text = "Complete list of all Python types that `cucumber` can serialize and deserialize.
...
These types are handled directly by `pickle` — `cucumber` passes them through.
...
Cucumber handles the container; contents are recursively serialized.
...
Cucumber handles circular references in:
