"""
Cucumber IR JSON Conversion Tests
"""
...
def _find_marker(node, marker: str) -> bool:
    if isinstance(node, dict):
        if node.get("__cucumber_json__") == marker:
            return True
...
def _find_cucumber_type(node, cucumber_type: str) -> bool:
    if isinstance(node, dict):
        if node.get("__cucumber_type__") == cucumber_type:
            return True
        return any(_find_cucumber_type(v, cucumber_type) for v in node.values())
    if isinstance(node, list):
        return any(_find_cucumber_type(item, cucumber_type) for item in node)
    return False
...
def test_ir_to_jsonable_contains_markers():
    ...
    assert _find_marker(jsonable, "dict") or _find_cucumber_type(jsonable, "dict"), \
        "Should represent dicts in JSON output"
    assert _find_marker(jsonable, "bytes"), "Should mark bytes values"
    assert _find_marker(jsonable, "set") or _find_cucumber_type(jsonable, "set"), \
        "Should represent sets in JSON output"
    assert _find_marker(jsonable, "tuple") or _find_cucumber_type(jsonable, "tuple"), \
        "Should represent tuples in JSON output"
    assert _find_marker(jsonable, "complex") or _find_cucumber_type(jsonable, "complex"), \
...
def test_serialize_ir_debug_verbose():
    ...
    ir = cucumber.serialize_ir({"a": 1}, debug=True, verbose=True)
    assert isinstance(ir, dict)


def test_to_json_debug_verbose():
    """to_json should support debug+verbose together."""
    json_text = cucumber.to_json({"a": 1}, debug=True, verbose=True)
    parsed = json.loads(json_text)
...
def test_to_jsonable_debug_verbose():
    ...
    jsonable = cucumber.to_jsonable({"a": 1}, debug=True, verbose=True)
...
def test_doc_to_jsonable_example():
    ...
    jsonable = cucumber.to_jsonable({"a": 1})
    assert isinstance(jsonable, (dict, list))


def test_doc_to_json_example():
    """Docstring example: to_json."""
    json_text = cucumber.to_json({"a": 1})
    parsed = json.loads(json_text)
    assert isinstance(parsed, dict)
...
def run_all_tests():
    runner = TestRunner("Cucumber IR JSON Tests")
