...
def test_now_and_get_current_time_basics():
    """Both return floats close to time.time() and are non-decreasing across calls."""
    t_lib = time.time()
    t1 = sktime.now()
    t2 = sktime.get_current_time()
    assert isinstance(t1, float)
    assert isinstance(t2, float)
    # within reasonable drift from immediate time.time() call
    _assert_between(abs(t1 - t_lib), 0.0, 0.1)
    _assert_between(abs(t2 - t_lib), 0.0, 0.1)
...
def test_timer_lap_and_stop_records_two_times():
    ...
    assert timer.slowest_lap in (0, 1)
    assert timer.fastest_lap in (0, 1)
