...
Here are the performance benchmarks for `cerial` compared to other serialization libraries.
...
```bash
python -m tests.cerial.run_all_benchmarks
```
...
### `suitkaise` specific types that only `cerial` can handle
...
`cerial` actually handles `requests.Session` 1.45x faster than base `pickle`.

## Primitive type comparison (excluding base `pickle`)

`bool`: `cerial` is fastest
- `cerial` 6.4µs 
- `cloudpickle` 10.4µs 
- `dill` 55.1µs

`int`: `cerial` is fastest
- `cerial` 1.2µs
- `cloudpickle` 2.6µs
- `dill` 10.7µs

`float`: `cerial` is fastest
- `cerial` 1.6µs
- `cloudpickle` 1.7µs
- `dill` 8.5µs

`complex`: `cerial` is fastest
- `cerial` 5.8µs
- `cloudpickle` 14.2µs
- `dill` 41.8µs

`str`: `cerial` is fastest
- `cerial` 1.0µs
- `cloudpickle` 1.5µs
- `dill` 7.3µs

`bytes`: `cerial` is fastest
- `cerial` 0.9µs
- `cloudpickle` 1.4µs
- `dill` 7.2µs
...
This is mainly because `cerial` fast paths basic types directly to the IR instead of handling them normally.

## `cerial` vs `cloudpickle` vs `dill`

### Types `cerial` handles the fastest (>1.33x faster)

Here is a table of types that `cerial` handles the fastest (>1.33x faster) compared to `cloudpickle` and `dill`, and how much faster `cerial` is than the second fastest serializer.

If a 2nd place speed is less than 1.33x faster than `cerial`, we consider this not significant enough to list, as times in microseconds (µs) are very small and can easily be affected by external factors.

────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  Type                                  speed with `cerial`              How much faster than 2nd place?
────────────────────────────────────────────────────────────────────────────────────────────────────────────────
...
───────────────────────────────────────────────────────────────────────────────────────────────────────────────
  `Skclass`                             9.8µs                            5.24x (`cerial`)
  `multiprocessing.Pipe`                6.6µs                            4.77x (`dill`)
  `SQLiteReconnector`                   5.1µs                            4.06x (`cerial`)
  `CodeType`                            9.4µs                            3.82x (`cerial`)
  `PurePath`                            3.8µs                            3.82x (`dill`)
  `SpooledTemporaryFile`                9.9µs                            3.69x (`cerial`)
  `Logger`                              4.0µs                            3.65x (`dill`)
  `SharedMemory`                        12.0µs                           3.61x (`dill`)
  `list`                                1.7µs                            3.29x (`cerial`)
  `CompletedProcess`                    5.8µs                            3.19x (`cerial`)
  `Enum`                                6.4µs                            2.95x (`dill`)
  `re.Pattern`                          6.4µs                            2.80x (`cerial`)
  `tuple`                               1.5µs                            2.73x (`cerial`)
  `Flag`                                5.1µs                            2.65x (`cerial`)
  `Skpath`                              8.5µs                            2.65x (`cerial`)
  `ModuleType`                          7.2µs                            2.58x (`dill`)
  `Class object`                        3.3µs                            2.58x (`cerial`)
  `set`                                 1.4µs                            2.50x (`cerial`)
  `OS pipes`                            1.7µs                            2.47x (`cerial`)
  `functools.partial`                   22.4µs                           2.46x (`cerial`)
  `date`                                4.0µs                            2.42x (`cerial`)
  `Skfunction`                          34.4µs                           2.40x (`cerial`)
  `frozenset`                           1.3µs                            2.31x (`cerial`)
  `typing.TypedDict`                    2.0µs                            2.25x (`cerial`)
  `enumerate`                           7.5µs                            2.23x (`cerial`)
  `dict`                                1.8µs                            2.06x (`cerial`)
  `timezone`                            6.6µs                            1.98x (`cerial`)
  `StringIO`                            5.8µs                            1.90x (`cerial`)
  `deque`                               5.7µs                            1.84x (`cerial`)
  `ErrorHandlerError`                   4.1µs                            1.78x (`cerial`)
  `PipeReconnector`                     6.2µs                            1.77x (`cerial`)
  `Path`                                4.0µs                            1.75x (`cerial`)
  `Formatter`                           7.3µs                            1.74x (`cerial`)
  `ProcessTimeoutError`                 4.1µs                            1.73x (`cerial`)
  `BytesIO`                             6.3µs                            1.73x (`cerial`)
  `FunctionTimeoutError`                3.7µs                            1.70x (`cerial`)
  `UUID`                                4.8µs                            1.69x (`cerial`)
  `CustomRoot`                          4.7µs                            1.68x (`cerial`)
  `Fraction`                            4.2µs                            1.67x (`cerial`)
  `DeserializationError`                4.2µs                            1.67x (`cerial`)
  `ProcessError`                        6.2µs                            1.66x (`cerial`)
  `range_iterator`                      8.5µs                            1.65x (`cerial`)
  `Class instance`                      4.2µs                            1.62x (`cerial`)
  `zip`                                 9.2µs                            1.60x (`cerial`)
  `PreRunError`                         5.0µs                            1.58x (`cerial`)
  `PathDetectionError`                  4.1µs                            1.54x (`cerial`)
  `ResultTimeoutError`                  4.0µs                            1.48x (`cerial`)
  `PostRunError`                        4.2µs                            1.45x (`cerial`)
  `ResultError`                         4.1µs                            1.44x (`cerial`)
  `SkModifierError`                     3.9µs                            1.44x (`cerial`)
  `timedelta`                           3.7µs                            1.43x (`cerial`)
  `PosixPath`                           3.5µs                            1.43x (`cerial`)
  `Decimal`                             4.6µs                            1.41x (`cerial`)
  `OnFinishError`                       4.1µs                            1.37x (`cerial`)
  `time`                                3.7µs                            1.35x (`cerial`)
...
`cerial` handles more types than `dill` and is almost always faster.

───────────────────────────────────────────────────────────────────────────────────────────────────────────────
  Type                                  speed with `dill`                How much faster than 2nd place?
───────────────────────────────────────────────────────────────────────────────────────────────────────────────
  `BufferedReader`                      28.7µs                           3.19x (`cerial`)
  `BufferedWriter`                      25.5µs                           1.84x (`cerial`)
  `FileIO`                              24.7µs                           1.81x (`cerial`)
