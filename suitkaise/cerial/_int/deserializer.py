"""
Central deserializer for cerial.
...
# - The IR is nested dicts/lists with metadata markers like "__cerial_type__"
...
# - Look for metadata markers: "__cerial_type__", "__handler__", "__object_id__"
...
# - When we encounter a "__cerial_ref__" marker, replace it with the placeholder
...
class Decerializer:
    ...
    def _reconstruct_recursive(self, ir_data: Any) -> Any:
        ...
        3. Checks if data has cerial metadata (needs handler reconstruction) - CHECK BEFORE collections!
    ...
    def _reconstruct_collection(self, data: Any) -> Any:
        ...
        Note: By the time we get here, we've already checked for cerial metadata,
        so all dicts here are plain dicts (no __cerial_type__ marker).
    ...
    def _has_cerial_metadata(self, data: Any) -> bool:
        """
        Check if data has cerial metadata markers.
        
        Two types of cerial objects:
    ...
    def _reconstruct_pickle_native(self, data: Dict[str, Any]) -> Any:
        ...
        Format: {"__cerial_type__": "pickle_native", "__object_id__": id, "value": obj}
        """
        obj_id = data.get("__object_id__")
        value = data["value"]
    ...
    def _reconstruct_pickle_native_func(self, data: Dict[str, Any]) -> Any:
        ...
        Format: {"__cerial_type__": "pickle_native_func", "__object_id__": id, "value": func}
        """
        obj_id = data.get("__object_id__")
        value = data["value"]
    ...
    def _reconstruct_wrapped_collection(self, data: Dict[str, Any]) -> Any:
        """
        Reconstruct a cerial-wrapped collection.
        
        The serializer wraps certain collections to preserve type and handle
        complex keys/values:
        - dict: {"__cerial_type__": "dict", "items": [(k, v), ...], "__object_id__"?}
        - tuple: {"__cerial_type__": "tuple", "items": [items], "__object_id__"?}
        - set: {"__cerial_type__": "set", "items": [items], "__object_id__"?}
        - frozenset: {"__cerial_type__": "frozenset", "items": [items], "__object_id__"?}
    ...
    def _log(self, message: str) -> None:
        ...
        if self.debug:
            print(f"[CERIAL DESERIALIZE] {message}", file=sys.stderr)
