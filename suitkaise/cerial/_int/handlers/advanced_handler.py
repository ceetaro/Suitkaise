...
class ContextVarHandler(Handler):
    """
    Serializes contextvars.ContextVar objects (2% importance).
    
    ContextVars provide context-local state (like threading.local but
    for async contexts).
    """
    ...
    def can_handle(self, obj: Any) -> bool:
        """Check if object is a ContextVar."""
        try:
            import contextvars
    ...
    def extract_state(self, obj: Any) -> Dict[str, Any]:
        ...
        - current_value: Current value in this context (if set)
        """
        import contextvars
        ...
        return {
            "name": obj.name,
            "has_value": has_value,
            "current_value": current_value,  # Will be recursively serialized
        }
    ...
    def reconstruct(self, state: Dict[str, Any]) -> Any:
        ...
        Create new ContextVar and set value if one existed.
        """
        import contextvars
        
        # Create new ContextVar
        var = contextvars.ContextVar(state["name"])
        
        # Set value if one existed
        if state["has_value"]:
