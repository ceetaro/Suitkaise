...
## pause: focus on pipe reconnection

Pipes start out as a pair of file descriptors or connection handles, that connect one endpoint to the other.

They can be one-way (one side writes and the other reads), or two-way (both sides can read and write).

So the question is: can we create a wrapper for both os file pipes and multiprocessing pipes that keeps pipes fast while giving them clearer, consistent semantics in our ecosystem?

Short answer: yes, but we intentionally do not support post-init reattachment. Pipes are born as paired endpoints with no address, so they only work when the peer is handed off at process start.

### Conceptual model (what gives pipes value in our ecosystem)

- The Pipe wrapper keeps pipes fast, predictable, and minimal.
- The parent process keeps the locked endpoint forever (the anchor).
- The peer endpoint is handed off only at process start (no post-init reattachment).
- This preserves pipe speed while keeping the API explicit.

### What we need to make this real

- A lock rule: locked endpoints cannot be serialized/transferred.
- A simple handoff path at process start (constructor args).
- Traceable errors when a pipe endpoint is used without a paired peer
  (clear exception with guidance: "peer endpoint was never handed off").

### Why pipes don't "reattach"

Pipes are just paired handles created together. There is no address or lookup for a lost peer, so we keep them limited to start-time handoff.
...
# returns Pipe.Anchor and Pipe.Point
point1, point2 = Pipe.pair()

# point1 is always locked in the process that created it
# this is the "anchor" endpoint (usually the parent process)

# pass point2 to another process (new memory space)
p = MyProcess(point2)

# wherever point2 gets passed at process start, it will be the new endpoint
# the parent keeps point1 so all endpoints always talk to the parent

# locking pipes in 2 points
# point1 is already locked in the main process

# and in MyProcess, we can...
class MyProcess(Skprocess):

    def __init__(self, pipe_endpoint: processing.Pipe.Point):

        # endpoint is handed off at process start only
        # no post-init reattachment
        self.pipe_endpoint = pipe_endpoint
        self.pipe_endpoint.lock()



# pipe points cannot be sent to share
```
