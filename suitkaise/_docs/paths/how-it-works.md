...
The `Skpath` class is an enhanced path object that wraps Python's `pathlib.Path` with extra features, including a cross-platform friendly path.

Initialize with:
- `path` : a string, `Path`, `Skpath`, ID generated by `Skpath.id`, or `None` (uses caller's file path)
...
The ID can be used to reconstruct the path later.
```python
path = Skpath("myproject/feature/file.txt")
id = path.id

exact_same_path = Skpath(id)
```
...
Returns an integer hash for use in sets and dict keys. This is not the same as the `Skpath.id` encoded ID.
...
2. `setup.sk` *file* - highest priority indicator (custom `suitkaise` marker file)
...
Process
1. Strip whitespace if `strip_whitespace` is enabled
...
5. Apply lowercase if `lowercase` is enabled
