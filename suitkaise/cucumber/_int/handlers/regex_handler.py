...
class MatchObjectHandler(Handler):
    ...
    def reconstruct(self, state: Dict[str, Any]) -> MatchReconnector:
        """
        Reconstruct match information.
        
        Returns a MatchReconnector. Call reconnect() to re-run the pattern
        on the original string and get a live re.Match object.
        """
        return MatchReconnector(state=state)
