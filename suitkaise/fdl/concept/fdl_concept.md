...
box color does not inherit from text color automatically, must be set itself
but you can do `</box ..., color current>` to get the same color
...
since we arent using defaults anymore, we can always just create a new blank formatting state for every print, and don't have to worry about things like resetting.
