...
2. Serialize with cucumber
3. Deserialize with cucumber
...
# Add parent directories to path to import cucumber
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

try:
    import cucumber
    CUCUMBER_AVAILABLE = True
except ImportError:
    CUCUMBER_AVAILABLE = False
    print("⚠️  cucumber not yet implemented - will simulate for demo")
...
def test_full_cycle_verbose():
    ...
    if not CUCUMBER_AVAILABLE:
        print("❌ cucumber not available - cannot test serialization")
        print("   Implement cucumber.serialize() first!")
        original.cleanup()
        return False
    
    try:
        print("Calling cucumber.serialize(original)...")
    ...
    try:
        print("Calling cucumber.deserialize(serialized_bytes)...")
...
def test_with_selective_types():
    ...
    if not CUCUMBER_AVAILABLE:
        print("❌ cucumber not available - cannot test")
        return False
