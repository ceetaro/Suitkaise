"""
Benchmark cucumber against pickle, cloudpickle, and dill.

Tests:
1. Simple objects (where pickle should win)
2. Medium complexity (comparable)
3. Complex objects (where only cucumber works)
"""
...
class BenchmarkRunner:
    ...
    def benchmark_serializer(self, name: str, obj: Any, num_iterations: int = 1000):
        ...
        """
        if name == "cucumber":
            serialize_func = self.cucumber_s.serialize
            deserialize_func = self.cucumber_d.deserialize
        elif name == "pickle":
    ...
    def run_benchmark_suite(self, suite_name: str, obj: Any, num_iterations: int = 1000):
        ...
        results = {}
        serializers = ["pickle", "dill", "cloudpickle", "cucumber"]
        ...
        if results["cucumber"]["success"] and results["pickle"]["success"]:
            speedup = results["pickle"]["ops_per_sec"] / results["cucumber"]["ops_per_sec"]
            print(f"\n  pickle is {speedup:.1f}x faster than cucumber (for this object)")
...
def create_complex_object():
    """Complex object - only cucumber can handle."""
    import threading
    import logging
    import io
...
def create_module_level_function():
    """
    Module-level function - tests cucumber's reference-based fast path.
...
def create_simple_instance():
    """
    Simple class instance - tests cucumber's simple_class_instance fast path.
    ...
    This triggers cucumber's flat IR format with no circular ref tracking.
    """
...
def main():
    ...
    print("="*70)
    print("\nComparing: pickle, dill, cloudpickle, cucumber")
    print("\nLibraries available:")
    ...
    # ==========================================================================
    # SECTION 2: Cucumber Fast Paths
    # ==========================================================================
    print("\n" + "="*70)
    print("SECTION 2: CUCUMBER FAST PATHS")
    print("="*70)
    ...
    print("="*70)
    print("\nTesting each cucumber handler in isolation...")
    ...
    cucumber_passed = sum(1 for r in runner.results.values() if r.get("cucumber", {}).get("success", False))
    pickle_passed = sum(1 for r in runner.results.values() if r.get("pickle", {}).get("success", False))
    
    print(f"\nTotal benchmarks: {total}")
    print(f"Cucumber success rate: {cucumber_passed}/{total} ({100*cucumber_passed/total:.0f}%)")
    ...
    print("   - cucumber trades some speed for universal capability")
    
    print("\n2. Cucumber Fast Paths:")
    ...
    print("   - cucumber: Slower but universal (100% of objects work)")
