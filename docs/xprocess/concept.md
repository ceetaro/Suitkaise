...
The XProcess API aims to provide clean, intuitive interfaces for process management:

```python
# Simple process creation
xp = xprocess.CrossProcessing()
process = xp.create_process("worker", worker_function, config={"join_after": 5})

# Context manager lifecycle hooks
...
# Class-based process definition
class MyProcess(xprocess.Process):
    def __beforeloop__(self):
        # Custom pre-loop logic
    def __afterloop__(self):
        # Custom post-loop logic
```

The API should feel natural and hide complexity while providing powerful process lifecycle management.